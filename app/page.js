...
function PortalCliente({cliente,onLogout}){
  ...
  const profsDoServico=selSrv?profs.filter(p=>!selSrv.tipo||p.tipo===selSrv.tipo):profs
  ...
  // Próximos 7 dias
  function getProxDias(){
    const dias=[]
    for(let i=0;i<14;i++){
      const d=new Date();d.setDate(d.getDate()+i)
      const iso=d.toISOString().slice(0,10)
      const dmy=isoToDmy(iso)
      const slots=getSlotsLivres(dmy)
      dias.push({iso,dmy,d,slots,livre:slots.length>0})
    }
    return dias
  }

  // Chips de horário por período
  function getPeriodos(slots){
    const m=slots.filter(h=>h<'12:00')
    const t=slots.filter(h=>h>='12:00'&&h<'18:00')
    const n=slots.filter(h=>h>='18:00')
    return[{l:'☀️ Manhã',s:m},{l:'🌤️ Tarde',s:t},{l:'🌙 Noite',s:n}].filter(p=>p.s.length>0)
  }

  function handleFoto(e){
    const f=e.target.files?.[0];if(!f)return
    const r=new FileReader()
    r.onload=ev=>{setFotoPreview(ev.target.result);setFoto(ev.target.result)}
    r.readAsDataURL(f)
  }

  async function confirmar(){
    if(!selSrv||!selProf||!selDmy||!selTime){setErr('Preencha todos os campos');return}
    ...
    setStep(1);setSelSrv(null);setSelProf(null);setSelDmy(todayStr());setSelTime('');setFoto(null);setFotoPreview(null)
    supabase.from('salon_bookings').select('*').eq('client_name',cliente.full_name).order('booking_date','desc').then(({data})=>setAgs(data||[]))
    setTimeout(()=>{setOk('');setTab('reservas')},2200)
  }

  const P=T.primary,PL=T.primaryLight,PP=T.primaryPale
  const dias=getProxDias()
  const slotsLivres=getSlotsLivres(selDmy)
  const periodos=getPeriodos(slotsLivres)
  ...
  // ── STEPS ────────────────────────────────────────────
  const stepTitles=['Serviço','Profissional','Data e Hora','Confirmação']
  ...
  return(
    <>
      ...
      <style>{`
        ...
        .pw{max-width:620px;margin:0 auto;padding:16px 16px 90px;}
        .bnav{position:fixed;bottom:0;left:0;right:0;background:#fff;border-top:1px solid #ede8df;
          display:flex;z-index:100;max-width:620px;margin:0 auto;}
        .bni{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;
          padding:10px 0 12px;gap:3px;background:none;border:none;cursor:pointer;
          font-family:'Manrope',sans-serif;font-size:10px;font-weight:600;color:#aaa;transition:color .18s;}
        .bni.on{color:${P};}
        .bni-ic{font-size:20px;line-height:1;}
        .srv-card{background:#fff;border-radius:14px;padding:16px;border:2px solid transparent;
          cursor:pointer;transition:all .18s;display:flex;align-items:center;gap:14px;margin-bottom:10px;}
        .srv-card:active{transform:scale(.98);}
        .srv-card.sel{border-color:${P};background:${PP};}
        .prof-card{background:#fff;border-radius:14px;padding:14px 16px;border:2px solid transparent;
          cursor:pointer;transition:all .18s;display:flex;align-items:center;gap:14px;margin-bottom:10px;}
        .prof-card.sel{border-color:${P};background:${PP};}
        .day-chip{flex-shrink:0;width:54px;padding:10px 0;border-radius:12px;border:2px solid #ede8df;
          background:#fff;cursor:pointer;text-align:center;transition:all .18s;font-family:'Manrope',sans-serif;}
        .day-chip.sel{border-color:${P};background:${PP};}
        .day-chip.no{opacity:.38;cursor:default;}
        .time-chip{padding:8px 14px;border-radius:20px;border:2px solid #ede8df;background:#fff;
          cursor:pointer;font-family:'Manrope',sans-serif;font-size:13px;font-weight:600;transition:all .18s;color:#333;}
        .time-chip.sel{border-color:${P};background:${P};color:#fff;}
        .step-bar{display:flex;gap:6px;margin-bottom:20px;}
        .step-dot{flex:1;height:4px;border-radius:2px;background:#ede8df;transition:background .3s;}
        .step-dot.on{background:${P};}
        .upload-area{border:2px dashed #d4ad65;border-radius:14px;padding:28px 20px;
          text-align:center;cursor:pointer;transition:all .18s;background:#fdf9f2;}
        .upload-area:hover{border-color:${P};background:${PP};}
      `}</style>
      ...
      <div className="pw">
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,paddingTop:4}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <div style={{width:42,height:42,borderRadius:'50%',background:`linear-gradient(135deg,${PL},${P})`,
              display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Noto Serif,serif',
              fontSize:18,fontWeight:700,color:'white'}}>{(cliente.full_name||'?')[0]}</div>
            <div>
              <div style={{fontFamily:'Noto Serif,serif',fontSize:17,fontWeight:600}}>Olá, {cliente.full_name.split(' ')[0]}!</div>
              <div style={{fontSize:11,color:T.onSurfaceLow}}>Portal do Cliente</div>
            </div>
          </div>
          <button onClick={onLogout} className="btn btn-ghost btn-sm">Sair</button>
        </div>
        ...
        {tab==='agendar'&&(
          <>
            ...
            <div className="step-bar">
              {stepTitles.map((_,i)=><div key={i} className={`step-dot${step>i?' on':''}`}/>)}
            </div>
            <div style={{fontSize:12,color:T.onSurfaceLow,marginBottom:16,display:'flex',alignItems:'center',gap:6}}>
              <span style={{fontWeight:700,color:P}}>Passo {step}/4</span>
              <span>·</span>
            ...
            {step===1&&(
              <div className="card au">
                <div className="card-hd"><span className="ch">O que você quer fazer hoje?</span></div>
                <div style={{padding:'0 16px 16px'}}>
                  {srvs.length===0&&<div style={{padding:20,textAlign:'center',color:T.onSurfaceLow,fontSize:13}}>Carregando serviços…</div>}
                  {srvs.map(s=>(
                    <div key={s.id} className={`srv-card${selSrv?.id===s.id?' sel':''}`} onClick={()=>setSelSrv(s)}>
                      <div style={{width:44,height:44,borderRadius:12,background:selSrv?.id===s.id?PP:'#f4f3f1',
                        display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,flexShrink:0}}>
                        {s.tipo==='cabelereiro'?'💇':(s.tipo==='manicure'||s.name?.toLowerCase().includes('unha')?'💅':'✨')}
                      </div>
                      <div style={{flex:1}}>
                        <div style={{fontSize:14,fontWeight:700,color:'#1a1c1a'}}>{s.name}</div>
                        <div style={{fontSize:11,color:T.onSurfaceLow,marginTop:2}}>{s.duration_min} min</div>
                      </div>
                      <div style={{textAlign:'right',flexShrink:0}}>
                        <div style={{fontFamily:'Noto Serif,serif',fontSize:16,fontWeight:700,color:P}}>{fmtCurrency(s.price)}</div>
                        {selSrv?.id===s.id&&<div style={{fontSize:10,color:P,marginTop:2}}>✓ Selecionado</div>}
                      </div>
                    </div>
                  ))}
                  <button className="btn btn-primary" style={{width:'100%',marginTop:8,justifyContent:'center',opacity:selSrv?1:.45}} 
                    onClick={()=>selSrv&&setStep(2)} disabled={!selSrv}>
                    Próximo →
                  </button>
                </div>
              </div>
            ...
            {step===2&&(
              <div className="card au">
                <div className="card-hd"><span className="ch">Com quem?</span></div>
                <div style={{padding:'0 16px 16px'}}>
                  {profsDoServico.map(p=>(
                    <div key={p.id} className={`prof-card${selProf?.id===p.id?' sel':''}`} onClick={()=>setSelProf(p)}>
                      <div style={{width:48,height:48,borderRadius:'50%',background:`linear-gradient(135deg,${PL},${P})`,
                        display:'flex',alignItems:'center',justifyContent:'center',
                        fontFamily:'Noto Serif,serif',fontSize:20,fontWeight:700,color:'white',flexShrink:0}}>
                        {(p.full_name||'?')[0]}
                      </div>
                      <div style={{flex:1}}>
                        <div style={{fontSize:14,fontWeight:700}}>{p.full_name}</div>
                        <div style={{fontSize:11,color:T.onSurfaceLow,marginTop:2}}>{p.specialty}</div>
                      </div>
                      {selProf?.id===p.id&&<div style={{fontSize:18}}>✓</div>}
                    </div>
                  ))}
                  <div style={{display:'flex',gap:10,marginTop:8}}>
                    <button className="btn btn-ghost" style={{flex:1,justifyContent:'center'}} onClick={()=>setStep(1)}>← Voltar</button>
                    <button className="btn btn-primary" style={{flex:1,justifyContent:'center',opacity:selProf?1:.45}}
                      onClick={()=>selProf&&setStep(3)} disabled={!selProf}>Próximo →</button>
                  </div>
                </div>
              </div>
            )}
            ...
            {step===3&&(
              <div className="card au">
                <div className="card-hd"><span className="ch">Quando?</span></div>
                <div style={{padding:'0 16px 16px'}}>
                  {/* Calendário semanal */}
                  <div style={{fontSize:11,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:P,marginBottom:10}}>Selecione o dia</div>
                  <div style={{display:'flex',gap:8,overflowX:'auto',paddingBottom:8,marginBottom:16}}>
                    {dias.map(d=>{
                      const wd=['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'][d.d.getDay()]
                      const day=d.d.getDate()
                      const isSel=selDmy===d.dmy
                      return(
                        <div key={d.iso} className={`day-chip${isSel?' sel':''} ${!d.livre?' no':''}`}
                          onClick={()=>{if(d.livre){setSelDmy(d.dmy);setSelTime('')}}}>
                          <div style={{fontSize:9,fontWeight:700,color:isSel?P:'#aaa'}}>{wd}</div>
                          <div style={{fontSize:17,fontWeight:700,color:isSel?P:'#333',marginTop:2}}>{day}</div>
                          {!d.livre&&<div style={{fontSize:8,color:'#ccc',marginTop:1}}>—</div>}
                        </div>
                      )
                    })}
                  </div>
                  ...
                  {slotsLivres.length===0
                    ?<div style={{padding:'16px 0',textAlign:'center',color:T.onSurfaceLow,fontSize:13}}>Sem horários disponíveis neste dia</div>
                    :periodos.map(per=>(
                      <div key={per.l} style={{marginBottom:14}}>
                        <div style={{fontSize:11,fontWeight:700,color:T.onSurfaceLow,marginBottom:8}}>{per.l}</div>
                        <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
                          {per.s.map(h=>(
                            <button key={h} className={`time-chip${selTime===h?' sel':''}`} onClick={()=>setSelTime(h)}>{h}</button>
                          ))}
                        </div>
                      </div>
                    ))
                  ...
                  <div style={{display:'flex',gap:10,marginTop:16}}>
                    ...
                    <button className="btn btn-primary" style={{flex:1,justifyContent:'center',opacity:selTime?1:.45}}
                      onClick={()=>selTime&&setStep(4)} disabled={!selTime}>Próximo →</button>
                  </div>
                </div>
              </div>
            )}

            {/* PASSO 4: Confirmação + Foto */}
            {step===4&&(
              <div className="card au">
                <div className="card-hd"><span className="ch">Confirmar pedido</span></div>
                <div style={{padding:'0 16px 16px'}}>
                  {/* Resumo */}
                  <div style={{background:PP,borderRadius:14,padding:'16px',marginBottom:16}}>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                      {[
                        {l:'Serviço',v:selSrv?.name},
                        {l:'Profissional',v:selProf?.full_name},
                        {l:'Data',v:selDmy},
                        {l:'Horário',v:selTime},
                        {l:'Valor',v:fmtCurrency(selSrv?.price||0)},
                        {l:'Duração',v:(selSrv?.duration_min||0)+' min'},
                      ].map(x=>(
                        <div key={x.l}>
                          <div style={{fontSize:9,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:P,opacity:.7}}>{x.l}</div>
                          <div style={{fontSize:13,fontWeight:600,marginTop:2,color:'#1a1c1a'}}>{x.v}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Upload foto */}
                  <div style={{fontSize:11,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:P,marginBottom:10}}>
                    📸 Foto de Referência <span style={{fontWeight:400,textTransform:'none',letterSpacing:0,color:T.onSurfaceLow}}>(opcional)</span>
                  </div>
                  <input ref={fileRef} type="file" accept="image/*" style={{display:'none'}} onChange={handleFoto}/>
                  <input ref={camRef} type="file" accept="image/*" capture="environment" style={{display:'none'}} onChange={handleFoto}/>
                  {!fotoPreview?(
                    <div className="upload-area" onClick={()=>fileRef.current?.click()}>
                      <div style={{fontSize:28,marginBottom:8}}>📎</div>
                      <div style={{fontSize:13,fontWeight:600,color:P}}>Arraste ou toque para adicionar</div>
                      <div style={{fontSize:11,color:T.onSurfaceLow,marginTop:4}}>A profissional verá sua referência</div>
                      <div style={{display:'flex',gap:10,justifyContent:'center',marginTop:12}}>
                        <button onClick={e=>{e.stopPropagation();fileRef.current?.click()}}
                          style={{padding:'7px 16px',background:PP,border:`1px solid ${P}`,borderRadius:8,
                            fontFamily:'Manrope,sans-serif',fontSize:12,fontWeight:600,color:P,cursor:'pointer'}}>
                          🖼️ Galeria
                        </button>
                    ...
                    <div style={{borderRadius:14,overflow:'hidden',border:`2px solid ${PP}`}}>
                      ...
                      <div style={{padding:'8px 14px',background:PP,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                        ...
                        <button onClick={()=>{setFoto(null);setFotoPreview(null)}}
                          style={{background:'none',border:'none',cursor:'pointer',fontSize:12,color:T.onSurfaceLow}}>Remover</button>
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{display:'flex',gap:10,marginTop:16}}>
                    <button className="btn btn-ghost" style={{flex:1,justifyContent:'center'}} onClick={()=>setStep(3)}>← Voltar</button>
                    <button className="btn btn-primary" style={{flex:2,justifyContent:'center',opacity:sending?.65:1}}
            ...
            <div style={{padding:'0 14px 14px'}}>
              {minhasAgs.length===0
                ?<div style={{padding:32,textAlign:'center',color:T.onSurfaceLow,fontSize:13}}>
                    ...
                    <div>Nenhuma reserva ainda</div>
                  </div>
                :minhasAgs.map(a=>{
                  const si=statusInfo(a.status)
                  return(
                    <div key={a.id} style={{display:'flex',alignItems:'center',gap:12,padding:14,
                      borderRadius:14,background:si.bg,marginBottom:10,border:`1px solid ${si.color}22`}}>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:14,fontWeight:700}}>{a.srvName}</div>
                        <div style={{fontSize:11,color:T.onSurfaceLow,marginTop:2}}>{a.profName} · {a.dmy} às {a.time}</div>
                        <div style={{fontSize:13,fontWeight:600,color:P,marginTop:4}}>{fmtCurrency(a.price)}</div>
                      </div>
                      <span style={{padding:'4px 10px',borderRadius:8,fontSize:10,fontWeight:700,
                        color:si.color,background:'rgba(255,255,255,.7)',flexShrink:0,whiteSpace:'nowrap'}}>{si.label}</span>
                    </div>
                  )
                })
              }
        ...
        {tab==='perfil'&&(
          <div className="card au">
            <div className="card-hd"><span className="ch">🔑 Alterar Senha</span></div>
            <div style={{padding:'0 16px 16px'}}>
      ...
      <nav className="bnav">
        {[
          {id:'agendar',ic:'✂️',l:'Agendar'},
          {id:'reservas',ic:'📅',l:'Reservas'},
