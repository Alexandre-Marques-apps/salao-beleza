...
function InspiracaoIA(){
  ...
  async function gerarInspiracao(){
    if(!descricao.trim()&&!fotoBase64){setErr('Descreva o estilo de unha ou envie uma foto');return}
    setErr('');setLoading(true);setImgUrl(null);setSugestao('')
    try{
      const res=await fetch('/api/inspiracao',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({descricao,fotoBase64})})
      const json=await res.json()
      if(json.ok&&json.imageData){setImgUrl(json.imageData);setSugestao(json.prompt||descricao)}
      else setErr('Não foi possível gerar. Tente novamente.')
    }catch(e){
      setErr('Erro de conexão. Tente novamente.')
    }
    setLoading(false)
  ...
  return(
    <div className="card au2" style={{marginBottom:16}}>
      <div className="card-hd"><span className="ch">💅 Inspiração de Unhas</span></div>
      <div style={{padding:'0 20px 20px'}}>
        ...
        <label className="lbl">Descreva o estilo de unha</label>
        <textarea
          value={descricao}
          onChange={e=>setDescricao(e.target.value)}
          placeholder="Ex: Unha stiletto vermelha com glitter, francesa nude, gel transparente..."
          className="inp"
