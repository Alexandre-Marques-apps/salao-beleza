...
function ProfPanel({prof,onLogout}){
  ...
  const tabs2=[{id:'pedidos',label:'Pedidos',ic:'🔔'},{id:'agenda',label:'Agenda',ic:'◷'},{id:'hoje',label:'Hoje',ic:'📅'},{id:'proximos',label:'Próximos',ic:'🗓'},{id:'agendar',label:'Agendar',ic:'➕'},{id:'rendimentos',label:'Ganhos',ic:'◎'},{id:'bloqueios',label:'Bloqueios',ic:'🚫'},{id:'senha',label:'Senha',ic:'🔑'}]
                      ...
                      :<div style={{display:'flex',flexDirection:'column',gap:6}}>
                        {slots.map(({h,ocupado})=>(
                          <div key={h} style={{display:'flex',alignItems:'center',gap:10,
                            ...
                            background:ocupado?T.primaryPale:T.surfaceLow,
                            border:ocupado?`1px solid ${T.primary}44`:'1px solid #e8e3db'}}>
                            <div style={{fontSize:14,fontWeight:700,color:ocupado?T.primary:T.onSurfaceLow,
                            ...
                            {ocupado?(
                              ...
                              <div style={{flex:1,fontSize:12,color:T.onSurfaceLow}}>Horário livre</div>
                            )}
...
function PortalCliente({cliente,onLogout}){
  ...
  async function confirmar(){
    ...
    setSelProf(null);setSelSrv(null);setSelDmy(todayStr());setSelTime('');setFotoPreview(null)
    supabase.from('salon_bookings').select('*').eq('client_name',cliente.full_name).order('booking_date','desc').then(({data})=>setAgs(data||[]))
