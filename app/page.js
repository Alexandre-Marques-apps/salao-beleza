...
// ══════════════════════════════════════════════════════
// COMPONENTE INSPIRAÇÃO COM IA
// ══════════════════════════════════════════════════════
function InspiracaoIA(){
  ...
  function handleFile(e,cam=false){
    const file=e.target.files?.[0]
    if(!file)return
    const reader=new FileReader()
    reader.onload=ev=>{
      setFotoBase64(ev.target.result.split(',')[1])
  ...
  async function gerarInspiracao(){
    if(!descricao.trim()&&!fotoBase64){setErr('Descreva o estilo desejado ou envie uma foto');return}
    setErr('');setLoading(true);setImgUrl(null);setSugestao('')
    try{
      const msgs=[{role:'user',content:fotoBase64
        ?[
            {type:'image',source:{type:'base64',media_type:'image/jpeg',data:fotoBase64}},
            {type:'text',text:`O cliente de salão de beleza enviou uma foto de referência${descricao?` e descreveu: "${descricao}"`:''}.
Crie um prompt em inglês simples e direto (máximo 20 palavras) para gerar uma imagem de inspiração de beleza baseada nisso.
Responda APENAS com o prompt em inglês, sem explicações.`}
          ]
        :[{type:'text',text:`Cliente de salão de beleza descreveu: "${descricao}".
Crie um prompt em inglês simples e direto (máximo 20 palavras) para gerar uma imagem de inspiração de beleza.
Responda APENAS com o prompt em inglês, sem explicações.`}]
      }]
      const apiRes=await fetch('https://api.anthropic.com/v1/messages',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:100,messages:msgs})
      })
      const apiJson=await apiRes.json()
      const prompt=apiJson.content?.[0]?.text?.trim()||descricao
      const promptEncoded=encodeURIComponent(`beauty salon hairstyle inspiration, ${prompt}, professional photo, high quality`)
      const url=`https://image.pollinations.ai/prompt/${promptEncoded}?width=512&height=512&nologo=true&seed=${Date.now()}`
      setImgUrl(url)
      setSugestao(prompt)
    }catch(e){
      const fbUrl=`https://image.pollinations.ai/prompt/${encodeURIComponent(`beauty salon inspiration, ${descricao||'beauty'}, professional photo, high quality`)}?width=512&height=512&nologo=true&seed=${Date.now()}`;setImgUrl(fbUrl);setSugestao(descricao)
    }
    setLoading(false)
  }

  function limpar(){
    setDescricao('');setImgUrl(null);setFotoBase64(null)
    setFotoPreview(null);setSugestao('');setErr('')
  }

  const primary='#775a19',pale='#f5edd8',low='#7a7a6a',surfLow='#f4f3f1'

  return(
    <div className="card au2" style={{marginBottom:16}}>
      <div className="card-hd"><span className="ch">✨ Inspiração de Estilo</span></div>
      <div style={{padding:'0 20px 20px'}}>
        <div style={{fontSize:12,color:low,marginTop:10,marginBottom:14,lineHeight:1.6}}>
          Descreva o estilo que você deseja ou envie uma foto de referência. Nossa IA vai gerar uma inspiração personalizada!
        </div>
        <label className="lbl">Descreva o estilo desejado</label>
        <textarea
          value={descricao}
          onChange={e=>setDescricao(e.target.value)}
          placeholder="Ex: Cabelo longo com luzes douradas, estilo natural e volumoso..."
          className="inp"
        ...
        <div style={{display:'flex',gap:10,marginTop:14,flexWrap:'wrap'}}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={()=>fileRef.current?.click()} style={{flex:1,justifyContent:'center'}}>
            📷 Enviar Foto
          </button>
          <button className="btn btn-ghost btn-sm" onClick={()=>camRef.current?.click()} style={{flex:1,justifyContent:'center'}}>
            📸 Usar Câmera
          </button>
        </div>
        {fotoPreview&&(
          <div style={{marginTop:12,borderRadius:12,overflow:'hidden',border:`2px solid ${pale}`}}>
            <img src={fotoPreview} alt="Referência" style={{width:'100%',maxHeight:180,objectFit:'cover',display:'block'}}/>
            <div style={{padding:'8px 12px',background:pale,fontSize:11,color:primary,fontWeight:600}}>
              ✓ Foto de referência carregada
              <button onClick={()=>{setFotoBase64(null);setFotoPreview(null)}} style={{marginLeft:10,background:'none',border:'none',cursor:'pointer',color:low,fontSize:11}}>Remover</button>
            </div>
          </div>
        )}
        ...
        <button
          ...
          style={{width:'100%',marginTop:16,justifyContent:'center',opacity:loading?.65:1}}
          onClick={gerarInspiracao}
          disabled={loading}
        >
          {loading?'Gerando inspiração…':'✨ Gerar Inspiração'}
        </button>
        {imgUrl&&(
          <div style={{marginTop:16}}>
            {sugestao&&<div style={{fontSize:11,color:low,marginBottom:8,fontStyle:'italic'}}>🎨 Prompt: {sugestao}</div>}
            <div style={{borderRadius:16,overflow:'hidden',boxShadow:'0 4px 20px rgba(119,90,25,.15)'}}>
              <img src={imgUrl} alt="Inspiração gerada" style={{width:'100%',display:'block'}} onError={()=>setErr('Erro ao carregar imagem. Tente novamente.')}/>
            </div>
            <div style={{display:'flex',gap:10,marginTop:12}}>
              <a href={imgUrl} download="inspiracao.jpg" className="btn btn-ghost btn-sm" style={{flex:1,justifyContent:'center',textDecoration:'none'}}>
                💾 Salvar Imagem
              </a>
              <button className="btn btn-primary btn-sm" style={{flex:1,justifyContent:'center'}} onClick={gerarInspiracao}>
                🔄 Gerar Outra
              </button>
              <button className="btn btn-danger btn-sm" onClick={limpar}>✕</button>
            </div>
          </div>
        )}
      </div>
