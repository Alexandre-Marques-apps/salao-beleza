...
function PortalCliente({cliente,onLogout}){
  ...
  const profsDoSrv=selSrv?profs.filter(p=>!selSrv.tipo||p.tipo===selSrv.tipo):profs
  ...
  async function confirmar(){
    ...
    setSelSrv(null);setSelProf(null);setSelDmy(todayStr());setSelTime('');setFotoPreview(null)
    supabase.from('salon_bookings').select('*').eq('client_name',cliente.full_name).order('booking_date','desc').then(({data})=>setAgs(data||[]))
  ...
  const P='#9A7D56',PA='#C5A880',PP='rgba(197,168,128,.12)',PB='rgba(197,168,128,.25)'
  const css=`
    ...
    .srv-card{background:#fff;border-radius:14px;padding:14px 16px;border:2px solid #eee;
      cursor:pointer;transition:all .18s;display:flex;justify-content:space-between;align-items:center;
    ...
    .upload-area{border:2px dashed ${PA};border-radius:14px;padding:24px 16px;
      text-align:center;cursor:pointer;transition:all .18s;background:#fdf9f3;position:relative;}
    ...
    .hdr{background:#fff;border-bottom:1px solid #f0ede8;position:sticky;top:0;z-index:100;
    ...
    <>
      ...
      <div className="hdr">
        <div style={{maxWidth:440,margin:'0 auto',padding:'14px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <div style={{width:40,height:40,borderRadius:'50%',background:`linear-gradient(135deg,${PA},${P})`,
          ...
          <button onClick={onLogout} style={{fontSize:11,fontWeight:700,color:'#e53e3e',
            background:'#fff5f5',border:'none',padding:'6px 14px',borderRadius:20,cursor:'pointer'}}>
            Sair
          </button>
        </div>
      </div>
      ...
      <div className="cpw" style={{marginTop:16}}>
        ...
        {tab==='agendar'&&(
          <div>
            ...
            {/* Info */}
            <div style={{background:'#fffbf2',border:'1px solid #f5e6c8',borderRadius:14,padding:'12px 14px',
            ...
            {/* 1. Serviço */}
            <div style={{marginBottom:20}}>
              <div className="lbl-s">✂️ 1. Escolha o serviço *</div>
              {srvs.length===0&&<div style={{padding:16,textAlign:'center',color:'#aaa',fontSize:13}}>Carregando…</div>}
              {srvs.map(s=>(
                <div key={s.id} className={`srv-card${selSrv?.id===s.id?' sel':''}`} onClick={()=>{setSelSrv(s);setSelProf(null);setSelTime('')}}>
                  <div>
                    <div style={{fontSize:13,fontWeight:700,color:'#222'}}>{s.name}</div>
                    <div style={{fontSize:11,color:'#aaa',marginTop:3}}>{s.duration_min} min</div>
                  </div>
                  <span style={{fontSize:14,fontWeight:800,padding:'5px 12px',borderRadius:20,
                    background:selSrv?.id===s.id?PA:'#f5f0ea',
                    color:selSrv?.id===s.id?'#fff':P}}>{fmtCurrency(s.price)}</span>
                </div>
              ))}
            </div>
            ...
            {selSrv&&(
              <div style={{marginBottom:20}}>
                <div className="lbl-s">👤 2. Escolha o profissional *</div>
                <div className="prof-grid">
                  {profsDoSrv.map(p=>(
                    <div key={p.id} className={`prof-card${selProf?.id===p.id?' sel':''}`} onClick={()=>{setSelProf(p);setSelTime('')}}>
            ...
            {selProf&&(
              <div style={{marginBottom:20}}>
                <div className="lbl-s">📅 3. Data do atendimento *</div>
            ...
            {selTime&&(
              <div style={{marginBottom:20}}>
                <div className="lbl-s">📎 5. Foto de referência <span style={{textTransform:'none',letterSpacing:0,fontWeight:400}}>(opcional)</span></div>
            ...
            {selSrv&&selProf&&selDmy&&selTime&&(
              <button onClick={confirmar} disabled={sending}
                style={{width:'100%',background:sending?'#ccc':`linear-gradient(135deg,${PA},${P})`,
                  color:'#fff',border:'none',borderRadius:14,padding:'15px',
                  fontFamily:'Manrope,sans-serif',fontSize:12,fontWeight:800,
                  letterSpacing:2,textTransform:'uppercase',cursor:sending?'not-allowed':'pointer',
                  boxShadow:'0 4px 16px rgba(197,168,128,.4)'}}>
                {sending?'Enviando…':'Confirmar e Enviar Solicitação'}
              </button>
            )}
          </div>
        )}
