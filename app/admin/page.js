...
export default function Admin() {
  ...
  // serviços compatíveis com o tipo do profissional
  function srvsCompativeis(nomeProf) {
    const p = profs.find(x => x.full_name === nomeProf)
    if(!p) return srvs
    return srvs.filter(s => (s.tipo||'cabelereiro') === (p.tipo||'cabelereiro'))
  }
  ...
  async function saveAg() {
    ...
    if(prof && srv && (prof.tipo||'cabelereiro') !== (srv.tipo||'cabelereiro')) {
      setFerr(`⚠️ ${form.professional_name} não realiza este serviço. Tipo incompatível.`); return
    }
    const payload = {
  ...
  async function saveSrv() {
    ...
    // define tipo automaticamente pela categoria
    const tipoUnha = ['Unhas','Estética']
    const tipo = tipoUnha.includes(form.categoria) ? 'manicure' : 'cabelereiro'
    const payload = {
      name:         form.name.trim(),
    ...
    <>
      ...
      {modal==='agendamento' && form.profissionalFixo && (
        <Modal title={`${form.professional_name} · ${form.start_time}`} onClose={closeModal}>
          ...
          <Sel value={form.service_name||''} onChange={F('service_name')}>
            ...
            {srvsCompativeis(form.professional_name).map(s=><option key={s.id}>{s.name}</option>)}
          </Sel>
      ...
      {modal==='agendamento' && !form.profissionalFixo && (
        <Modal title={form.id?'Editar Agendamento':'Novo Agendamento'} onClose={closeModal}>
          ...
          <Sel value={form.professional_name||''} onChange={v=>{setForm(f=>({...f,professional_name:v,service_name:''}));setFerr('')}}>
            <option value="">Selecionar...</option>
            {profs.map(p=><option key={p.id}>{p.full_name}</option>)}
          </Sel>
          <Lbl>Serviço * {form.professional_name && <span style={{fontSize:10,color:'rgba(0,0,0,.4)',textTransform:'none',letterSpacing:0}}>— compatível com {form.professional_name.split(' ')[0]}</span>}</Lbl>
          <Sel value={form.service_name||''} onChange={F('service_name')}>
            <option value="">Selecionar...</option>
            {srvsCompativeis(form.professional_name).map(s=><option key={s.id}>{s.name}</option>)}
          </Sel>
      ...
      {modal==='profissional' && (
        <Modal title={form.id?'Editar Profissional':'Novo Profissional'} onClose={closeModal}>
          ...
          <Box cor="azul">
            {(form.tipo||'cabelereiro')==='cabelereiro'
              ? '✂️ Poderá realizar: Corte, Barba, Coloração, Escova e Tratamentos'
              : '💅 Poderá realizar: Manicure, Pedicure e Estética'}
          </Box>
